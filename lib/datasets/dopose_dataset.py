...
def compute_xyz(depth_img : np.ndarray, camera_params : dict) -> np.ndarray:
    ...
    px, py = camera_params["x_offset"], camera_params["y_offset"]
    fx, fy = camera_params["fx"], camera_params["fy"]

    indices = np.indices((img_height, img_width), dtype=np.float32).transpose(1,2,0) # [H,W,2]
...
class DoPoseDataset(data.Dataset, datasets.imdb):

    def __init__(self, image_set : str, dopose_dataset_path : str = None):

        self._name = "dopose_dataset_" + image_set
        self._image_set = image_set
        self._dopose_dataset_path = self._get_default_path() if dopose_dataset_path is None else dopose_dataset_path
        ...
        indices_file = os.path.join(self._dopose_dataset_path, "{}.npy".format(self._image_set))
        ...
        camera_params_file = os.path.join(self._dopose_dataset_path, "camera_params.json")
        assert os.path.exists(camera_params_file), "{} does not exist".format(camera_params_file)
    ...
    def _get_default_path(self):
        return os.path.join(datasets.ROOT_DIR, 'data', 'dopose')

    def __getitem__(self, index: int):

        file_index = self._indices[index]
        rgb_file = os.path.join(self._dopose_dataset_path, "rgb", "{}.png".format(file_index))
        depth_file = os.path.join(self._dopose_dataset_path, "depth", "{}.png".format(file_index))
