...
def parse_args():
    ...
    parser.add_argument(
        "--solver", dest="solver", help="solver type", default="sgd", type=str
    )
    parser.add_argument(
        "--dataset",
        dest="dataset_name",
    ...
    parser.add_argument(
        "--dataset-val",
        dest="dataset_val_name",
        help="dataset to perform validation on while training",
        default="ucn_syn_dataset_val",
        type=str,
    )
    parser.add_argument(
        "--dataset_background",
        dest="dataset_background_name",
        help="background dataset to train on",
        default="background_nvidia",
        type=str,
    )
    parser.add_argument(
        "--rand",
    ...
    parser.add_argument(
        "--cad", dest="cad_name", help="name of the CAD files", default=None, type=str
    )
    parser.add_argument(
        "--pose",
        dest="pose_name",
...
if __name__ == "__main__":
    ...
    val_dataset = get_dataset(args.dataset_val_name)
    ...
    if len(cfg.INTRINSICS) > 0:
        ...
        val_dataset._intrinsic_matrix = K
    ...
    network = networks.__dict__[args.network_name](
        dataset.num_classes, cfg.TRAIN.NUM_UNITS, network_data
    ).cuda()
    ...
    param_groups = [
        {
            "params": network.module.bias_parameters(),
            "weight_decay": cfg.TRAIN.WEIGHT_DECAY,
        },
        {
            "params": network.module.weight_parameters(),
            "weight_decay": cfg.TRAIN.WEIGHT_DECAY,
        },
    ]
    ...
    if args.solver == "adam":
        ...
        optimizer = torch.optim.SGD(
            param_groups, cfg.TRAIN.LEARNING_RATE, momentum=cfg.TRAIN.MOMENTUM
        )
    ...
    for epoch in range(args.startepoch, args.epochs):
        ...
        if (epoch + 1) % cfg.TRAIN.SNAPSHOT_EPOCHS == 0 or epoch == args.epochs - 1:
            ...
            infix = (
                "_" + cfg.TRAIN.SNAPSHOT_INFIX if cfg.TRAIN.SNAPSHOT_INFIX != "" else ""
            )
            filename = (
