...
def calc_network_size(network):
    ...
    print("model_size = {:.3f}MB".format(total_size / (1024 ** 2)))
...
def read_sample(filename_color, filename_depth, camera_params):
    ...
    if cfg.INPUT == "DEPTH" or cfg.INPUT == "RGBD":
        # depth image
        # depth_img = np.load(filename_depth)
        ...
        # fx = camera_params["fx"]
        # fy = camera_params["fy"]
        # if "cx" in camera_params.keys():
        #     px = camera_params["cx"]
        # else:
        #     px = camera_params["x_offset"]
        # if "cy" in camera_params.keys():
        #     py = camera_params["cy"]
        # else:
        #     py = camera_params["y_offset"]
        # xyz_img = compute_xyz(depth, fx, fy, px, py, height, width)

        xyz_img = np.load(filename_depth)
        xyz_img = np.nan_to_num(xyz_img)
        # xyz_img = xyz_img / 1000.
    else:
        xyz_img = None
    ...
    # im = im[234:926, 442:1536, :]
    # depth_img = depth_img[234:926, 442:1536]
    # im = im[234:726, 442:1036, :]
    # if xyz_img is not None:
...
if __name__ == "__main__":
    ...
    if args.pretrained_crop:
        network_data_crop = torch.load(
            args.pretrained_crop, map_location=torch.device("cpu")
        )
    ...
    for i in index_images:
        if os.path.exists(images_color[i]):
            ...
            # run network
            out_label, out_label_refined = test_sample(sample, network, network_crop, ind=i)
        else:
